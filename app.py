# app.py — WWASD Relay v2.5
# Port: hardened SSR + resilient /blofin/latest (dict, never raises)
# TV: restored read endpoints (/tv/latest, /snap) without touching ingest or alerts
...
# TV freshness window (seconds). Default matches prior desk expectations.
FRESH_CUTOFF_SECS = int(os.getenv("FRESH_CUTOFF_SECS", "5400"))  # 90 minutes

# Port freshness (seconds) — default 240s as we hardened earlier
...
@app.get("/tv/latest")
async def tv_latest(request: Request, list: Optional[str] = None, lists: Optional[str] = None,
    ...
    'lists' is accepted as an alias; if comma-separated, returns the multi-list
    structure identical to /snap (for backward compatibility).
    """
    # alias/compat
    name = list or lists or ""
    if name and "," in name:
        # multi → mirror /snap
        return await snap(name, fresh_only=fresh_only, max_age_secs=max_age_secs)
...
async def snap(lists: str = "green,macro,full", fresh_only: int = 1, max_age_secs: int = FRESH_CUTOFF_SECS):
    wanted = [x.strip().lower() for x in lists.split(",") if x.strip()]
    if not wanted:
        wanted = ["green"]
    resp: Dict[str, Any] = {"ts": now_ms(), "lists": {}}
    for name in wanted:
        data = await tv_latest(Request, list=name, fresh_only=fresh_only, max_age_secs=max_age_secs)  # type: ignore
        # tv_latest returns {"count":..,"items":[...]}
        resp["lists"][name] = data
    return resp

# Optional HTML mirror for quick human checks
...
async def blofin_latest():
    ...
    age_ok = _fresh_ms(int(ts) if ts else None, BLOFIN_TTL_SEC)
    return {"fresh": bool(age_ok), "ts": int(ts) if ts else None, "data": _blofin_latest}

# -------------------- Port SSR (unchanged shape) --------------------
def _fmt(v: Any) -> str:
