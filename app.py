...
# ===================== WWASD TV state (TradingView) =====================
from cachetools import TTLCache

# Hold last N TV STATE messages for up to TV_TTL_S seconds (default 24h)
TV_TTL_S = int(os.getenv("TV_TTL_S", "86400"))
TV_FRESH_WINDOW_S_DEFAULT = int(os.getenv("TV_FRESH_WINDOW_S", "900"))  # 15 minutes

tv_mem = TTLCache(maxsize=10000, ttl=TV_TTL_S)  # symbol -> state dict
tv_last = {"ts": 0}

def _load_tv_lists():
    """
    Try several locations for lists; allow missing files.
    Expected JSON shape if found: {"green": ["BTCUSDT.P", ...], "full": [...], "macro": [...]}
    Also supports simple newline-delimited text files under ./lists/{green,full,macro}.txt
    """
    import json
    import pathlib

    # 1) JSON file path via env or common names
    candidates = [
        os.getenv("WWASD_LISTS_FILE"),
        "tv_lists.json",
        "lists/tv_lists.json",
        "_archive/tv_lists.json",
    ]
    for p in candidates:
        if p and os.path.exists(p):
            try:
                ...
                return {k.lower(): [s.strip() for s in v] for k, v in d.items() if isinstance(v, list)}
            except Exception:
                pass

    # 2) Plain text fallback
    lists = {}
    for name in ("green", "full", "macro"):
        p = os.path.join("lists", f"{name}.txt")
        if os.path.exists(p):
            with open(p, "r", encoding="utf-8") as fh:
                syms = [ln.strip() for ln in fh if ln.strip() and not ln.startswith("#")]
                lists[name] = syms

    return lists

TV_LISTS = _load_tv_lists()  # may be {}
TV_LISTS_VERSION = int(time.time())

def _is_fresh(ts_ms: int, fresh_window_s: int) -> bool:
    try:
        # Some Pine payloads send time() seconds; others ms as string. Normalize.
        if ts_ms is None:
            return False
        ts = int(ts_ms)
        # Heuristic: if it's 10-digit assume seconds, convert to ms
        if ts < 10_000_000_000:
            ts *= 1000
        return (int(time.time() * 1000) - ts) <= (fresh_window_s * 1000)
    except Exception:
        return False
...
async def tv_ingest(request: Request, token: str):
    """
    Ingest WWASD_State_Emitter alerts (JSON body) from TradingView.
    Requires token == AUTH_SHARED_SECRET. Stores by symbol in tv_mem and updates tv_last.
    """
    ...
    t = body.get("type") or body.get("t") or ""
    if t.upper() not in {"WWASD_STATE", "BLOFIN_POSITIONS"}:
        # Permit other types but only store WWASD_STATE into tv_mem
        return JSONResponse({"ok": True, "ignored": True})

    global tv_last
    body.setdefault("server_received_ms", int(time.time() * 1000))
    tv_last = body

    if t.upper() == "WWASD_STATE":
...
@app.get("/tv/last")
async def tv_last_event():
    """Return the most recent TV event (any type) the server saw."""
    return JSONResponse(tv_last if tv_last else {"ts": 0})

@app.get("/tv/latest")
async def tv_latest(lists: str = "green", fresh_only: int = 1, fresh_window_s: int = TV_FRESH_WINDOW_S_DEFAULT):
    """
    Return recent per-symbol state for the requested list(s).
    Query params:
      lists=green,full,macro (comma separated)
      fresh_only=1 to filter to events within fresh_window_s (default 900s)
      fresh_window_s=900 override
    Response shape:
      {"ts": 1234567890, "lists": {"green": {"count": N, "items": [...]}, "full": {...}}}
    """
    now_ms = int(time.time() * 1000)
    req_lists = [s.strip().lower() for s in (lists or "").split(",") if s.strip()]
    if not req_lists:
        req_lists = ["green"]

    # Fallback: if we don't have configured lists, build from what we've seen so far
    available_keys = set(tv_mem.keys())
    lists_map = dict(TV_LISTS)  # shallow copy
    if not lists_map.get("green") and available_keys:
        lists_map["green"] = sorted(available_keys)
    if not lists_map.get("full") and available_keys:
        lists_map["full"] = sorted(available_keys)
    if not lists_map.get("macro"):
        # Allow macro to be empty; callers must handle
        lists_map.setdefault("macro", [])

    payload = {"ts": now_ms, "lists": {}}
    for name in req_lists:
        syms = lists_map.get(name, [])
        items = []
        for s in syms:
            state = tv_mem.get(s)
            if not state:
                continue
            fresh = _is_fresh(state.get("ts"), fresh_window_s)
            if fresh_only and not fresh:
                continue
            # Return a compact subset that your TV/Desk expects
            items.append({
                "type": state.get("type"),
                ...
                "mtf": state.get("mtf"),
                "is_fresh": fresh,
                "server_received_ms": state.get("server_received_ms"),
            })
        payload["lists"][name] = {"count": len(items), "items": items}
